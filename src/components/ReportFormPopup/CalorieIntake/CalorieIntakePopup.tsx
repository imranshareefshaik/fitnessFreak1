...
const CalorieIntakePopup: React.FC<CaloriIntakePopupProps> = ({ setShowCalorieIntakePopup }) => {
  ...
  const saveCalorieIntake = async () => {
    ...
    .then(data => {
      if (data.ok) {
        toast.success('Calorie intake addes successfully')
        getCalorieIntake()
      }
      else {
...
const getCalorieIntake = async () => {
  setItems([])
  fetch(process.env.NEXT_PUBLIC_API + '/calorieintake/getcalorieintakebydate', {
    method: 'POST' ,
    headers: {
      'Content-Type': 'application/json',
  },
...
.catch(err => {
  ...
  const deleteCalorieIntake = async (item: any) => {
    fetch(process.env.NEXT_PUBLIC_API + '/calorieintake/deletecalorieintake', {
      method: 'DELETE' ,
      headers: {
        'Content-Type': 'application/json',
    },
  ...
  .then(data => {
    if(data.ok) {
      toast.success('Calorie intake item deleted sucessfully')
      getCalorieIntake()
    } 
    else {
      toast.error('Error in getting calorie intake')
